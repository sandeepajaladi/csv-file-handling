...
def test_calculator_add_result():
    ...
    test_data = pd.read_csv(r'C:\Users\jaladis\PycharmProjects\csv-file-handling\testdata\Addition.csv').data
    for row in test_data:
        result = float(row['Result'])
        # assert Calculator.add(row['Number2'], row['Number1']) == result
...
def test_calculator_subtract_result():
    """testing calculator result is 0"""
    test_data = pd.read_csv('testdata/Subtraction.csv').data
    for row in test_data:
        result = float(row['Result'])
        # assert Calculator.subtract(row['Number2'], row['Number1']) == result
...
def test_calculator_multiply_result():
    """testing calculator result is 0"""
    test_data = pd.read_csv('testdata/Multiplication.csv').data
    for row in test_data:
        result = float(row['Result'])
        # assert Calculator.multiply(row['Number2'], row['Number1']) == result
...
def test_calculator_divide_result():
    """testing calculator result is 0"""
    test_data = pd.read_csv('testdata/Division.csv').data
    for row in test_data:
        result: float = float(row['Result'])
        assert Calculator.divide(row['Number2'], row['Number1']) == row[result]
